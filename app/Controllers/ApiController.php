...
class ApiController {
    ...
    /**
     * Retourne le token plan pour un concours (si présent en session via lien email)
     */
    private function getPlanTokenForConcours($concoursId) {
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
...
/*                        elseif (isset($attachment['url'])) {
    ...
    /**
     * Proxy pour /api/concours/categories-classement (concour_categories_classement filtré par iddiscipline)
     * Query: ?iddiscipline=X ou ?iddiscipline=1,2,3
     */
...
console.log($response);
    ...
    public function proxyConcoursPlanPeloton($concoursId) {
        if (!$this->isAuthenticated()) {
            $this->sendUnauthenticatedResponse();
            return;
        }
        try {
            $method = $_SERVER['REQUEST_METHOD'];
            $queryString = $_SERVER['QUERY_STRING'] ?? '';
            $endpoint = "concours/{$concoursId}/plan-peloton" . ($queryString ? "?{$queryString}" : "");
            $data = null;
            if ($method === 'POST' || $method === 'PUT' || $method === 'PATCH') {
                $input = file_get_contents('php://input');
                $data = json_decode($input, true);
