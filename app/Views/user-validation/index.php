            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <?php if (empty($deletionPendingUsers)): ?>
                        <div class="text-center py-5">
                            <i class="fas fa-check-circle fa-3x text-success mb-3"></i>
                            <h4 class="text-muted">Aucun utilisateur en attente de suppression</h4>
                            <p class="text-muted">Tous les comptes sont actifs.</p>
                        </div>
                    <?php else: ?>
                        <div class="alert alert-warning">
                            <i class="fas fa-exclamation-triangle me-2"></i>
                            <strong>Attention :</strong> Ces utilisateurs ont validé leur demande de suppression de compte. 
                            Vous devez procéder à la suppression définitive dans les 30 jours conformément au RGPD.
                        </div>
                        <div class="table-responsive">
                            <table class="table table-hover">
                                <thead class="table-light">
                                ...
                                <tbody>
                                    <?php foreach ($deletionPendingUsers as $user): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php 
                                                $validatedAt = $user['deletion_validated_at'] ?? $user['updated_at'] ?? '';
                                                if ($validatedAt) {
...
<!-- Modales de suppression pour chaque utilisateur en attente de suppression -->
<?php if (!empty($deletionPendingUsers)): ?>
    <?php foreach ($deletionPendingUsers as $user): ?>
        <div class="modal fade" id="deleteModal<?php echo $user['id'] ?? ''; ?>" tabindex="-1">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form method="POST" action="/user-validation/delete">
                        <div class="modal-body">
                            <div class="alert alert-danger">
                                <i class="fas fa-exclamation-circle me-2"></i>
                                <strong>Action irréversible !</strong> Vous êtes sur le point de supprimer définitivement le compte de 
                                <strong><?php echo htmlspecialchars($user['name'] ?? ''); ?></strong>.
                                <br><br>
                                Cette action supprimera :
                                <ul class="mb-0 mt-2">
                                    <li>Toutes les données personnelles de l'utilisateur</li>
                                    <li>Son historique d'entraînements</li>
                                    <li>Ses documents et fichiers</li>
                                    <li>Tous ses messages et participations</li>
                                </ul>
                            </div>
                            ...
                            <div class="mb-3">
                                <label class="form-label">
                                    <strong>Pour confirmer, tapez le nom d'utilisateur :</strong>
                                </label>
                                <input type="text" 
                                       class="form-control" 
                                       ...
                                       required>
                                <small class="text-muted">Tapez exactement : <code><?php echo htmlspecialchars($user['username'] ?? ''); ?></code></small>
                            </div>
                        </div>
