...
$allConcours = is_array($concours ?? null) ? $concours : [];
$concoursActifs = [];
$concoursPasses = [];
$todayTs = strtotime(date('Y-m-d'));

foreach ($allConcours as $item) {
    $dateReference = $item['date_fin'] ?? $item['date_debut'] ?? null;
    $dateReferenceTs = $dateReference ? strtotime((string)$dateReference) : false;
    $isPast = ($dateReferenceTs !== false && $dateReferenceTs < $todayTs);
...
<div class="container-fluid" data-concours-index data-user-id="<?= htmlspecialchars($_SESSION['user']['id'] ?? '') ?>">
    ...
    <div class="row">
        <div class="col-12">
            ...
            <?php if (empty($concoursActifs) && empty($concoursPasses)): ?>
                <div class="card">
            ...
            <?php else: ?>
                <?php if (!empty($concoursActifs)): ?>
                <div class="card">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <h5 class="card-title mb-0">
                                <i class="fas fa-calendar-check me-2"></i>Concours a venir / en cours
                            </h5>
                            <span class="badge bg-primary" id="clubsCount">
                                <?php echo count($concoursActifs); ?> concours<?php echo count($concoursActifs) > 1 ? 's' : ''; ?>
                            </span>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover" id="concoursTable">
                                ...
                                <tbody>
                                    <?php foreach ($concoursActifs as $item):
                                    $user = $_SESSION['user'] ?? [];
                ...
                <?php if (!empty($concoursPasses)): ?>
                <div class="card mt-4">
                    <div class="card-header">
                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                            <h5 class="card-title mb-0">
                                <i class="fas fa-calendar-times me-2"></i>Concours passes
                            </h5>
                            <span class="badge bg-secondary">
                                <?php echo count($concoursPasses); ?> concours<?php echo count($concoursPasses) > 1 ? 's' : ''; ?>
                            </span>
                        </div>
                    </div>
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-hover" id="concoursTablePast">
                                <thead>
                                    <tr>
                                        <th class="sortable" data-column="club" style="cursor: pointer;">
                                            Club <i class="fas fa-sort ms-1"></i>
                                ...
                                <tbody>
                                    <?php foreach ($concoursPasses as $item):
                                    $user = $_SESSION['user'] ?? [];
                                    $isAdmin = $user['is_admin'] ?? false;
                                    $isDirigeant = ($user['role'] ?? '') === 'Dirigeant';
                                    $canEditClub = $isAdmin || $isDirigeant || ($belongsToClub ?? false);
